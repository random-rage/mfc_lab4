...
std::string Blowfish::Encrypt(const std::string &src) const {
  ...
  for (int i = 0; i < dst.size() / sizeof(uint64_t); ++i) {
    data = const_cast<char *>(dst.data());
    uint32_t *left = &reinterpret_cast<uint32_t *>(data)[i * 2];
    uint32_t *right = &reinterpret_cast<uint32_t *>(data)[i * 2 + 1];
    EncryptBlock(left, right);
  }
...
std::string Blowfish::data2hex(std::string data) const {
  std::stringstream ss;
  
  for(size_t i = 0; i < data.size(); ++i)
    ss << std::internal << std::setfill('0') << std::setw(2) << std::hex << (data.data())[i] + 128;
  
  return ss.str();
}

std::string Blowfish::hex2data(const std::string& hex) const {
  std::string bytes;
  
  for (unsigned int i = 0; i < hex.length(); i += 2) {
    std::string byteString = hex.substr(i, 2);
    char byte = (char)(strtol(byteString.c_str(), NULL, 16) - 128);
